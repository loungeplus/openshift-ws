...
*Topology* (トポロジー) Viewで `EventListener` 用の新しい デプロイメント *el-nationalparks* が作成されていることを確認します。
...
以下のリンクをクリックして、Gitea 上のコードリポジトリに移動します。
...
画面の右上にある Settings (設定) リンクをクリックします。

[.bordershadow]
image::nationalparks-codechanges-settings.png[link="self",window=_blank]

*Webhooks* をクリックし、次に *Add Webhook* (Webhookを追加) ボタンをクリックし、最後に *Gitea* を選択します。
...
"Target URL" (ターゲット URL) 欄にリンクを貼り付けます。

`POST Content Type` を `application/json` に変更します。

最後に、*Update Webhook* (Webhookを追加)をクリックします。
...
link:{ocp4_starter_gitea_url}/{ocp4_starter_gitea_user}/nationalparks/src/branch/master/src/main/java/com/openshift/evg/roadshow/parks/rest[gitea リポジトリー,window='_blank']にある `BackendController.java` ファイルを開きましょう。
...
*Topology* (トポロジー) Viewで `nationalparks` をクリックし、*Resources* タブの *Builds* セクションを確認するか、以下のコマンドを実行して確認します。
