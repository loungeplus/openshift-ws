...
S2Iの詳細な説明はこのワークショップの範囲を超えていますが、link:{ocp4_starter_openshift_docs}/html/images/creating-images[OpenShift S2I ドキュメント,window='_blank']
...
Topology viewで、ヘッダーの *+* ボタンをクリックし、*Import from Git* (Gitからのインポート)を選択します。
...
OpenShift はソースコードで使用されているプログラミング言語を自動的に判別します。*Import Strategy* (インポートストラテジー)を選択することもできます。

*Edit Import Strategy* (インポートストラテジーの編集)をクリックします。
...
Builder Image (ビルダーイメージ)として *Java* が選択されていることを確認し、OpenJDK 11を使用するためにバージョン *openjdk-11-ubi8* を選択してください。

*General* (一般)セクションまでスクロールダウンし、以下の項目を追加します。
...
*Deploy* (デプロイ)セクションで *Deployment* を選択します。

*Advanced Options* (詳細オプション) で *Create a route* (route の作成) が *チェック* されていることを確認します。今度はこのビューから直接、別の *Secure Route* を作成します。

*Show advanced Routing options* (詳細なルーティングオプションの表示) をクリックします。すべてのオプションをデフォルトのままにして、*Security* (セキュリティー) セクションに移動します。*Secure Route* (セキュアなルート) オプションにチェックを入れます。*TLS termination* (TLS終端)で *Edge* を選択します。
...
下にスクロールして *Labels* (ラベル)セクションを展開し、3 つのラベルを追加します。
...
ビルド ログを表示するには、Topology viewで `nationalparks` エントリをクリックし、*Resources* (リソース) タブの *Builds* (ビルド) セクションで *View Logs* (ログの表示) をクリックします。
