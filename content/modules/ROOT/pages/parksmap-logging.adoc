...
まず第一に、Webコンソールまたはコマンドラインから *Pod* のログを直接確認できる機能です。
...
*Topology* viewで `parksmap` エントリをクリックし、*Resources* (リソース)タブをクリックします。*Pod* エントリの横に *View Logs* (ログの表示)リンクが表示されます。

[.bordershadow]
image::parksmap-view-logs-link.png[link="self",window=_blank]

*Pod* のログを表示するには、*View Logs* (ログの表示)リンクをクリックします。
