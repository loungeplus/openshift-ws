...
この出力は、現在 1 つの *Pod* がデプロイされる予定であることを示しています。 （`Desired`）が 1 に設定されており、実際にデプロイされている *Pod* は 1 つ（`Current`）です。この必要数を変更することで、OpenShift に必要となる *Pod* の数を増減させることができます。
...
または、*Developer パースペクティブ* で最大 2 つのPodにスケールアップすることもできます。 Topology（トポロジー）View から、`parksmap` デプロイメント構成をクリックし、*Details* (詳細)タブを選択します。
...
次のような出力が表示されます。*Service* が 2 つの *Endpoints* (エンドポイント)をターゲットにしていることに注意してください。
